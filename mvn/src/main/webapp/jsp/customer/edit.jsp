					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<div style="float:right;text-align: center;margin-right: 50px" >
							<div style="align-content:space-around;">
								<div>
                                    <img id="custPic" src="/pic/${customer.customerDetail.custPic}" height="200px" width="200px"  />
                                    <input type="hidden" name="customerDetail.custPic" value="${customer.customerDetail.custPic}">
                                </div>
								<div style="padding-top:10px"><input id="pictureFile" type="file" name="pictureFile" onchange="refresh_picture()"/></div>
							</div>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							...
							<TR>
								...
								<INPUT class=textbox id="customerBasic.custName"
											style="WIDTH: 180px" maxLength=50 name="customerBasic.custName" value="${customer.customerBasic.custName }">
								</td>
								<td>客户性别：</td>
								<td>
									<c:if test="${customer.customerBasic.custSex eq '男'}">
										<input type="radio" id="customerBasic.custSex" name="customerBasic.custSex" value="男" checked="checked">男
									...
									<c:if test="${customer.customerBasic.custSex eq '女'}">
										...
										<input type="radio" id="customerBasic.custSex" name="customerBasic.custSex" value="女" checked="checked">女
									</c:if>
								...
								<%--<INPUT class=textbox id="customerBasic.custLevel"--%>
									...
									<select name="customerBasic.custLevel['key']">
										<c:forEach items="${customerLevel}" var="custLevel">
											<option value="${custLevel['key']}" <c:if test="${custLevel['key'] eq customer.customerBasic.custLevel['key']}"> selected="selected" </c:if> >${custLevel['value']}</option>
										</c:forEach>
									</select>
								</td>
								<td>客户联系人：</td>
								<td>
                                    <select id="customerBasic.custLinkman.linkId" name="customerBasic.custLinkman.linkId" <c:if test="${sessionScope.user.userLinkman ne null}"> onfocus="this.defaultIndex=this.selectedIndex;" onchange="this.selectedIndex=this.defaultIndex;" </c:if>  >
							...
							<TR>
								...
								<td>
									<INPUT class=textbox id="custPhone"
										   style="WIDTH: 180px" maxLength=50 name="customerBasic.custPhone" value="${customer.customerBasic.custPhone}">
								</td>
								<td>客户信息来源 ：</td>
								<td>
									...
									<%--<INPUT class=textbox id="customerBasic.custSource['value']"--%>
										...
										<select name="customerBasic.custSource['key']">
											<c:forEach items="${customerSource}" var="custSource">
												<option value="${custSource['key']}" <c:if test="${custSource['key'] eq customer.customerBasic.custSource['key']}"> selected="selected" </c:if> >${custSource['value']}</option>
											</c:forEach>
										</select>
								</td>
							</TR>
							...
							<TR>
								...
								<td>
									<INPUT class=textbox id="customerDetail.custAddress"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custAddress" value="${customer.customerDetail.custAddress}">
								</td>
								<td>客户所属行业 ：</td>
								<td>
									...
									<select name="customerBasic.custIndustry['key']">
										<c:forEach items="${customerIndustry}" var="custIndustry">
											<option value="${custIndustry['key']}" <c:if test="${custIndustry['key'] eq customer.customerBasic.custIndustry['key']}"> selected="selected" </c:if> >${custIndustry['value']}</option>
										</c:forEach>
									</select>
								</td>
							</TR>
							<TR>
								...
								<INPUT class=textbox id="customerDetail.custRegion"
														style="WIDTH: 180px" maxLength=50 name="customerDetail.custRegion" value="${customer.customerDetail.custRegion}">
								</td>
								<td>客户邮政编码 ：</td>
								<td>
								<INPUT class=textbox id="customerDetail.custZip"
														style="WIDTH: 180px" maxLength=50 name="customerDetail.custZip" value="${customer.customerDetail.custZip}">
								</td>
							</TR>
							...
							<TR>
								...
								<td>
									<INPUT class=textbox id="customerDetail.custWebsite"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custWebsite" value="${customer.customerDetail.custWebsite }">
								</td>
								<td>客户营业执照注册号 ：</td>
								<td>
									<INPUT class=textbox id="customerDetail.custLicence"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custLicence" value="${customer.customerDetail.custLicence}">
								</td>
							</TR>
							...
							<TR>
								...
								<td>
									<INPUT class=textbox id="customerDetail.custCorporation"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custCorporation" value="${customer.customerDetail.custCorporation }">
								</td>
								<td>客户注册资金 ：</td>
								<td>
									<INPUT class=textbox id="customerDetail.custCapital"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custCapital" value="${customer.customerDetail.custCapital}">
								</td>
							</TR>
							...
							<TR>
								...
								<td>
									<INPUT class=textbox id="customerDetail.custBank"
										   style="WIDTH: 180px" maxLength=50 name="customerDetail.custBank" value="${customer.customerDetail.custBank }">
								</td>
							</TR>
							...
							<TR>
								<td>客户简介 ：</td>
								<td colspan="4">
									<textarea class=textbox id=sChannel2  style="OVERFLOW:hidden;width:400px;height:200px"
